...
class SP_Team_Free_Scripts{
	...
	public function __construct() {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'sp_team_free_admin_scripts' ) );
	}
	 ...
	 * Admin Scripts
	 */
	public function sp_team_free_admin_scripts() {
		// CSS Files
		wp_enqueue_style( 'team-free-meta-style', SP_TEAM_FREE_URL . 'inc/metabox/assets/css/admin.css', false, $this->version );
		wp_enqueue_style( 'wp-color-picker', false, $this->version );

		//JS Files
		wp_enqueue_script( 'wp-color-picker', array( 'jquery' ), $this->version, false );
		wp_enqueue_script( 'team-free-meta-js', SP_TEAM_FREE_URL . 'inc/metabox/assets/js/admin.js', array(	'jquery' ), $this->version, false );
	}
